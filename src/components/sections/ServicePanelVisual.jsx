    ...
    >
      <defs>
        <linearGradient id="wf-fade" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#fb923c" stopOpacity={active ? 0.75 : 0.5} />
      ...
      <circle cx="200" cy="230" r={active ? 4 : 3} fill="#fb923c">
...
function Graph({ offset, active }) {
  ...
  return (
    ...
    >
      ...
      <path d={path} fill="none" stroke="#fb923c" strokeOpacity={active ? 0.85 : 0.55} strokeWidth={active ? 2 : 1.5}>
      ...
      <path d={path} fill="none" stroke="#fb923c" strokeOpacity="0.12" strokeWidth="6" filter="blur(2px)" />
      <circle r={active ? 5 : 4} fill="#fb923c">
        <animateMotion dur={active ? '2.2s' : '4s'} repeatCount="indefinite" path={path} />
      </circle>
      {active && (
        <circle r="3" fill="#fdba74" opacity="0.7">
          <animateMotion dur="2.2s" begin="1.1s" repeatCount="indefinite" path={path} />
        </circle>
      )}
      {active &&
        [40, 160, 280, 360].map((cx, i) => (
          <circle key={cx} cx={cx} cy={[150, 140, 50, 30][i]} r="2.5" fill="#fdba74">
...
function Matrix({ offset, active }) {
  ...
  return (
    <div
      className="grid h-full w-full grid-cols-10 gap-[2px] font-mono text-[10px] leading-3 text-accent-400/40"
    ...
    >
      {Array.from({ length: columns }).map((_, col) => (
        <div
          key={col}
          className={`flex flex-col items-center overflow-hidden opacity-70 ${active ? 'text-accent-100/70' : ''}`}
